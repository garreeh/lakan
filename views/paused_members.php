...
<head>
	...
	<title>Lakan | Members (Paused)</title>
				...
				<div class="pagetitle">
					...
					<nav>
						<ol class="breadcrumb">
							...
							<li class="breadcrumb-item active">Members (Paused)</li>
						</ol>
					</nav>
								...
								<div class="col-xl-12 col-lg-12">
									<div class="tab-pane fade show active" id="aa" role="tabpanel" aria-labelledby="aa-tab">
										...
										<div class="table-responsive">
											<div id="modalContainerPausedCustomers"></div>

											<table class="table custom-table table-hover" name="paused_cus_table" id="paused_cus_table">
...
<script>
	...
	$('.toggle-sidebar-btn').click(function () {
		$('#paused_cus_table').css('width', '100%');
		// console.log(table) //This is for testing only
	});

	//Table for Supplier
	$(document).ready(function () {
		var paused_cus_table = $('#paused_cus_table').DataTable({
			"pagingType": "numbers",
			"processing": true,
			"serverSide": true,
			"ajax": "./../controllers/tables/paused_cus_table.php",
			"order": [
				[0, "desc"]
	...
	$(document).ready(function () {
		...
		$('#paused_cus_table').on('click', 'tr td:nth-child(8) .fetchDataUser', function () {
			...
			$.ajax({
				...
				success: function (response) {
					$('#modalContainerPausedCustomers').html(response);
	...
	$(document).ready(function () {
		// Function to handle click event on datatable rows
		$('#paused_cus_table').on('click', 'tr td:nth-child(8) .fetchDataUserDelete', function () {
			var user_id = $(this).closest('tr').find('td').first().text(); // Get the user_id from the clicked row
			console.log('Button clicked, User ID: ' + user_id);
	...
	$(document).ready(function () {
		// Function to handle click event on datatable rows
		$('#paused_cus_table').on('click', 'tr td:nth-child(4) .fetchDataPassword', function () {
			var user_id = $(this).closest('tr').find('td').first().text(); // Get the user_id from the clicked row
			console.log('Button clicked, User ID: ' + user_id);
